...
如果你开启本地自动上传，数据同步是“本地覆盖服务器”。适合你把本地后台当作主控台使用。

不要同时在阿里云后台新增数据，否则下一次本地同步会覆盖服务器上的新增数据。

如果你主要在线上后台操作，建议关闭本地自动上传，让阿里云 SQLite 作为主库。本地需要查看线上数据时，执行：

```bash
cd /Users/t/mail
./scripts/sync_sqlite_from_server.sh
```

脚本会先备份本地旧库，再把服务器数据库拉回本地。
